...
from training.dvngcf_trainer import DVngcfTrainer
...
def adding_args(parser):
    #training
    parser.add_argument('--model', type=str, default='dvngcf', metavar='N',
    ...
    parser.add_argument('--big_data', type=str, default='gowalla', metavar='N',
                        help='amazon-book,ml-100k,gowalla')
...
def load_bigdata(args, dataset_name, generator):
    logging.info("load big data. dataset_name = ... %s" % dataset_name)

    args.big_data = generator.train_items
    args.inner_test_data = generator.test_set
    args.n_big = generator.n_train
...
def create_model(args, model_name, user_num, item_num, adj_mat):
    ...
    if model_name == 'dvngcf':
        model = NGCF(user_num, item_num, adj_mat, args)
        trainer = DVngcfTrainer(model, args)
    else:
...
def init_training_device(process_ID, fl_worker_num, gpu_num_per_machine):
    ...
    device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu") #str(process_gpu_dict[process_ID - 1]) if torch.cuda.is_available() else "cpu")
...
if __name__ == "__main__":
    ...
    str_process_name = "GDVE:" + "diff_big_10" #str(process_id)
    ...
    #load_big_data
    big_path = './../../../data/gowalla'
    big_generator = Data_generator(big_path, args.batch_size)
    load_bigdata(args, args.big_data, big_generator)
