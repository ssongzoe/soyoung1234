...
from FedML.fedml_core.non_iid_partition.noniid_partition import partition_class_samples_with_dirichlet_distribution, partition_with_dirichlet_distribution
...
class Data_generator(object):
    ...
    def split_bigdata(self, train_items):
        ...
        for u, i in train_items.items():
            ...
            if curr == 0:
                ...
                # test_item = np.random.choice(rest_item, len(rest_item)*4 // (10 - big_size), replace=False) #20%
                # train_item = np.setdiff1d(rest_item, test_item)

                big_dict[u] = big_item
                # inner_test_dict[u] = test_item
                train_dict[u] = rest_item #rest_item, train_item

                n_big += len(big_item)
                n_train += len(rest_item) #len(rest_item) len(train_item)
    ...
    def partition_data(self, client_number, uniform=True):
        ...
        # gdve 쓸지 말지 용도
        new_train, train_items = self.split_bigdata(train_items)
        ...
        if uniform:
            ...
            for k, v in train_items.items():
                train_dict[k] = len(v)
            clients_idxs = partition_with_dirichlet_distribution(train_dict, client_number)
            # print(f"client_idxs, {clients_idxs}")
    ...
    def load_partition_data(self, client_number, uniform=True):
        ...
        for client in range(client_number):
            ...
            data_local_num_dict[client] = len(list2)#train_dataset_client)
            ...
            logging.info("Client idx = {}, local sample number = {}".format(client, len(list2)))#train_dataset_client)))
    ...
    def graph_partitioning(self, client_number, uniform=True):
        n_valid = 0
        valid_items = {}
        ...
        # auxiliary big data: total client data = 6:4
        new_train, train_items = self.split_bigdata(train_items)
        ...
        # non-iid : node class gaussian으로 뽑기
        else:
            idx_batch_per_client = partition_with_dirichlet_distribution(train_dict, client_number)
            clients_idxs = idx_batch_per_client
        ...
        for client in range(client_number):
            ...
            for user in clients_idxs[client]:
                # client_train_datasets[user] = train_items[user]
                sub_dist.append(len(train_items[user]))
            sub_dist.sort()
            dist_list.append(sub_dist)
            plt.xlim(0,180)
            plt.ylim(0,100)
            plt.hist(sub_dist, color= "skyblue")
            plt.title(f'sub_dist_{client+1}_client, n: {len(sub_dist)}')
            plt.show()
        # print(dist_list)
        for i in range(len(dist_list)):
            for j in range(len(dist_list)):
                if len(dist_list[i]) != len(dist_list[j]):
                    continue
                mutual_info = metrics.adjusted_mutual_info_score(dist_list[i], dist_list[j])
                print(f'{mutual_info} for ({i},{j})')
            print('---- ---- ---- ---- ---- ----')
...
if __name__ == "__main__":
    ...
    batch_size = 64
    data_generator = Data_generator(data_path, batch_size)
    # data_generator.load_partition_data(10, False)
    # data_generator.partition_data(10, False)
    data_generator.graph_partitioning(9, False)
