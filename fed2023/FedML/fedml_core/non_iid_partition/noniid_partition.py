...
def partition_with_dirichlet_distribution(train_dict, client_num):
    # np.random.shuffle(idx_k)
    alpha = 5
    train_dict = train_dict
    sorted_dict = sorted(train_dict.items(), key=lambda x: x[1])
    idx_new = [x[0] for x in sorted_dict]
    random.shuffle(idx_new, lambda: np.random.dirichlet(np.repeat(alpha, client_num))[0])


    idx_batch = np.array_split(idx_new, client_num)
    # min_size = min([len(idx_j) for idx_j in idx_batch])
    # print(f'data number = { [len(idx_j) for idx_j in idx_batch]}')
